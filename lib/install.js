...
function install (fs, gyp, argv, callback) {
  ...
  gyp.opts['ninja-path'] = gyp.opts['ninja-path'] || path.resolve(gyp.devDir, 'ninja');
  var ninjaPath = gyp.opts['ninja-path'];

  gyp.opts['ninja-url'] = gyp.opts['ninja-url'] || 'https://github.com/martine/ninja/archive/v1.4.0.tar.gz';
  const ninjaUrl = gyp.opts['ninja-url'];
  ...
  downloadNinja((err, alreadyExists) => {
    ...
    if (alreadyExists) {
      log.verbose('ninja', 'looks like already installed');
      processInstall();
    } else {
      log.verbose('ninja', 'building...');
      console.log('before ninja')
      buildNinja(processInstall);
      console.log('after ninja')
    }
  });
  ...
  function downloadNinja(done) {
    ...
    console.log(3)
    var requestOpts = { strictSSL: false, url: ninjaUrl };
    console.log(4)
    ...
    if (proxyUrl) {
      if (/^https?:\/\//i.test(proxyUrl)) {
        log.verbose('ninja', 'using proxy url: "%s"', proxyUrl)
        requestOpts.proxy = proxyUrl
      } else {
        log.warn('ninja', 'ignoring invalid "proxy" config setting: "%s"', proxyUrl)
      }
    }
    console.log(6)
    request(requestOpts)
      .on('response', function (res) {
        console.log(7)
        if (res.statusCode !== 200) {
          callback(new Error(res.statusCode + ' status Ninja downloading tarball'));
          return;
        }
        console.log(8)
        ...
        function logger(path) {
          console.log(path)
          var name = path.substring(ninjaPath.length + 1).replace(/[.]+?\//, '');
          if(name.length > 0) {
        ...
        const extracter = tar.extract({ path: ninjaPath, strip: 1, filter: logger });
        console.log(extracter)
        res.pipe(gunzip).pipe(extracter).on('end', function(err) {
          log.verbose('ninja', 'successfully downloaded');
          done(err);
        });
        console.log(13)
      })
  ...
  function buildNinja(done) {
    console.log('hello')
    var bootstrap = spawn('python', ['configure', '--bootstrap'], { cwd: ninjaPath, env: process.env });
    console.log('goodbye')
    bootstrap.on('close', function(code) {
      if(code !== 0) {
        log.info('NINJA', 'Usually it caused when no build toolchain found');
        log.info('NINJA', 'Check for `g++` or `cl.exec` in PATH or:');
        log.info('NINJA', ' - for Windows specify `VCINSTALLDIR` env variable');
        log.info('NINJA', ' - for non-Windows specify `CXX` env variable');

        return done(new Error('Failed to build Ninja!'));
      }
    ...
    bootstrap.stderr.on('data', function(data) {
      log.error('ninja bootstrap', data.toString().trim());
    });
  }
