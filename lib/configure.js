...
function configure (gyp, argv, callback) {
  ...
  function createConfigFile (err, vsInfo) {
    ...
    variables.nodelibdir = path.resolve(nodeDir, defaults.default_configuration)

    // set the node development directory
    variables.nodedir = nodeDir

    // explicit specify generators in config
    variables.generators = [];

    if(gyp.opts.ninja !== undefined) {
      variables.generators.push('ninja');
    }

    if(gyp.opts.makefile !== undefined) {
      variables.generators.push('make');
    }

    if(gyp.opts.msvs !== undefined) {
      variables.generators.push('msvs');
    }

    if (variables.generators.length === 0 && !~argv.indexOf('-f') && !~argv.indexOf('--format')) {
      log.verbose('gyp', 'gyp format was not specified; forcing "ninja"');
      // force the 'ninja' target for non-Windows
      variables.generators.push('ninja');
    }
  ...
  function runGyp (err) {
    ...
    // load config and strip comments
    const config = fs.readFileSync(configPath, { encoding: 'UTF-8' }).replace(/^#.+$/gm, '');
    const variables = JSON.parse(config).variables;
    
    variables.generators.forEach((generator) => {
      argv.push('-f', generator);
    })
